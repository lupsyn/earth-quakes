package com.challange.hilt.bdd.matchers
    ...
    fun requestMethodIs(method: HttpMethod): Matcher<RecordedRequest> {
        return object : TypeSafeDiagnosingMatcher<RecordedRequest>() {
            override fun matchesSafely(
                ...
                if (!match) {
                    mismatchDescription.appendText("request HTTP method was ")
                        .appendValue(recordedRequest.method)
                }
    ...
    fun withQueryParam(queryKey: String, queryValue: String): Matcher<RecordedRequest> {
        return object : TypeSafeDiagnosingMatcher<RecordedRequest>() {
            override fun matchesSafely(
                ...
                if (!found) {
                    if (uriValue == null) {
                        mismatchDescription.appendText("query parameter ").appendValue(queryKey)
                            .appendText(" is not present in the path")
                    } else {
                        mismatchDescription.appendText("the query parameter has the value ")
                            .appendValue(uriValue).appendText(" instead of ")
            ...
            override fun describeTo(description: Description) {
                description.appendText("a request that contains a query param ")
                    .appendValue(queryKey).appendText(" with value ").appendValue(queryValue)
            }
        }
    }
